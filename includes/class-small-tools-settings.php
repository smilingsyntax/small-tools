...
class Small_Tools_Settings {
    ...
    public function generate_settings_file() {
        ...
        $content = "<?php\n";
        $content .= "// This file is automatically generated by Small Tools plugin\n";
        ...
        foreach ($settings as $key => $value) {
            ...
            if (is_array($value)) {
                ...
                $content .= "define('$constant_name', '" . addslashes($serialized_value) . "');\n";
            } else {
                $content .= "define('$constant_name', '" . addslashes($value) . "');\n";
            }
        }
        ...
        if (!empty($settings['small_tools_selection_color'])) {
            $content .= "add_action('wp_head', function() {\n";
            $content .= "    echo '<style>\n";
            $content .= "    ::selection {\n";
            $content .= "        background-color: " . esc_attr($settings['small_tools_selection_color']) . ";\n";
            $content .= "        color: " . esc_attr($settings['small_tools_selection_text_color']) . ";\n";
            $content .= "    }\n";
        ...
        if ($settings['small_tools_disable_gutenberg'] === 'yes') {
            $content .= "add_filter('use_block_editor_for_post_type', '__return_false', 100);\n";
        } else {
            $disabled_post_types = (array) get_option('small_tools_gutenberg_disabled_post_types', array());
            if (!empty($disabled_post_types)) {
                $content .= "add_filter('use_block_editor_for_post_type', function(\$use_block_editor, \$post_type) {\n";
                $content .= "    \$disabled_types = " . var_export(array_values($disabled_post_types), true) . ";\n";
                $content .= "    return in_array(\$post_type, \$disabled_types) ? false : \$use_block_editor;\n";
                $content .= "}, 100, 2);\n\n";
            }
        }
